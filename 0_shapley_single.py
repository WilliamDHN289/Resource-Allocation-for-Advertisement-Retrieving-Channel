...
def surprise_svd_recommendation_sparse(sparse_matrix, top_n=100):
    ...
    ratings = np.ravel(data[rows, cols])
...
def run_ad_auction(recommend_data, sep_data, gamma, bidding_strategy, bidding_env):
    ...
    for idx, ads in winning_ads.items():
        print(f"Rank {idx + 1} winner ad: {ads}")
    '''
...
##################################################
# Part 4: Shapley Value Allocation 
##################################################
def shapley_value_allocation(ad_data, sparse_matrix, gamma, bidding_strategy, bidding_env, top_n=100):
    ...
    # Compute the reward for each subset with uniform contribution (revenue or score)
    ...
    factorial_N = math.factorial(num_channels)


    # Update the sharpely weight phi_i for each channel
    for i, ch in enumerate(channels):
        ...
        for r in range(len(other_channels) + 1):
            for subset in itertools.combinations(other_channels, r):
                ...
                # delta = revenue_S_union - revenue_S    # Use revenue as rewards function
                delta = score_S_union - score_S      # Use R as rewards function
                weight_factor = (math.factorial(len(S)) * math.factorial(num_channels - len(S) - 1)) / factorial_N
                marginal_contribution = weight_factor * delta
                shapley_values[ch] += marginal_contribution
    ...
    if total_phi == 0:
        normalized_weights = {ch: 1.0 / num_channels for ch in channels}
    else:
        normalized_weights = {ch: shapley_values[ch] / total_phi for ch in channels}
    
    # Compute the final revenue and score based on the shapely weight
    rec_results_final = {}
    for ch in channels:
...
if __name__ == "__main__":
    ...
    output_dir = '/Users/denghaonan/Desktop/AuctionNet-main/0_channel/output_single'
    ...
    cumulative_revenue_channels = {ch: 0 for ch in channels}
    cumulative_score_channels = {ch: 0 for ch in channels}
    cumulative_avg_pvalue_channels = {ch: 0 for ch in channels}
    ...
    for base_dir in base_dirs:
        ...
        for i in range(48):
            ...
            for ch in channels:
                rec_result = channel_functions[ch](sparse_matrix, top_n)
                channel_data = ad_data[ad_data['ad_index'].isin(rec_result)].copy()

                final_avg_pvalue = channel_data['pValue'].mean()
                final_revenue, final_score = run_ad_auction(channel_data, [channel_data], gamma, bidding_strategy, bidding_env)
                
                cumulative_revenue_channels[ch] += final_revenue
                cumulative_score_channels[ch] += final_score
                cumulative_avg_pvalue_channels[ch] += final_avg_pvalue

   
    labels = channels
    revenue_values = [cumulative_revenue_channels[ch] for ch in channels]
    '''
    plt.figure(figsize=(10, 6))
    x = np.arange(len(labels))
    plt.bar(x, revenue_values, width=0.6)
    plt.xlabel('Channel')
    plt.ylabel('Cumulative Revenue')
    plt.title('Cumulative Revenue by Channel')
    plt.xticks(x, labels)
    plt.grid(axis='y')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'cumulative_revenue_by_channel.png'))
    plt.close()
    
    # 绘制累计得分柱状图
    score_values = [cumulative_score_channels[ch] for ch in channels]
    plt.figure(figsize=(10, 6))
    plt.bar(x, score_values, width=0.6)
    plt.xlabel('Channel')
    plt.ylabel('Cumulative Score')
    plt.title('Cumulative Score by Channel')
    plt.xticks(x, labels)
    plt.grid(axis='y')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'cumulative_score_by_channel.png'))
    plt.close()
    '''
    
    # Finally output
    print("Cumulative Revenue by Channel:")
    for ch in channels:
        print(f"{ch}: {cumulative_revenue_channels[ch]}")
    print("Cumulative Score by Channel:")
    for ch in channels:
        print(f"{ch}: {cumulative_score_channels[ch]}")
    print("Cumulative average pValue by Channel:")
    for ch in channels:
        print(f"{ch}: {cumulative_avg_pvalue_channels[ch]}")
...
if __name__ == "__main__":
    ...
    output_dir = '/Users/denghaonan/Desktop/AuctionNet-main/0_channel/output_10Channels'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    ...
    print(f"Score of shapley: {score_bench_final}")
'''
